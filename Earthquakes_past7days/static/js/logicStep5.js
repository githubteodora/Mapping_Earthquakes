...
// change the basic marker to a circleMarker by using the pointToLayer function.
  ...
  for (var i = 0; i < magnitudes.length; i++) {  // After we iterate through the magnitudes, we'll add the color and text to the div element using div.innerHTML +=
    console.log(colors[i]);
    div.innerHTML +=
      "<i style='background: " + colors[i] + "'></i> " +  // For each iteration, we'll add a color from the colors array by styling the background of an <i> tag with color options
      magnitudes[i] + (magnitudes[i + 1] ? "&ndash;" + magnitudes[i + 1] + "<br>" : "+");  // add the interval between earthquake magnitudes for our colors
    }
    return div;  // The last thing we need to do is style the legend using CSS. Below the JavaScript code for the legend is the CSS code.
  };
